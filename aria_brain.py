    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if "content" not in data:
                return str(data.get("error", {}).get("message", str(data)))
...
async def register_webhook():
    ...
    if token:
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                await client.post(f"https://api.telegram.org/bot{token}/setWebhook", json={"url": webhook_url, "drop_pending_updates": False})
        except Exception:
            pass
